...
import { ShaderNode, ShaderNodeObject } from 'three/src/nodes/TSL.js'
import {
  atan,
  clamp,
  color,
...
// Hard constants
const PULSES_FREQUENCY = 40
const PULSES_SPEED = 3.2
const GOD_RAYS_ANGULAR_SCALE = 10.0
const GOD_RAYS_RADIAL_SCALE = 32.0

// Water tuning
// const WATER_HEIGHT_SCALE = 0.05; // visual displacement scale
const WATER_IMPACT_DEPTH = 0.24 // impulse strength
const WATER_VISCOSITY = 0.93 // damping
...
export const MagicPlane: FC = () => {
  ...
  const { colorNode, opacityNode, uPlayerPosition, uSpeedSmooth, computeHeight, uLightAmount } =
    useMemo(() => {
      // Pulsating Rings
      ...
      const uPlayerPosition = uniform(vec4(...PLAYER_INITIAL_POS, 0.0))
      ...
      const getNeighborValues = (
        index: ShaderNodeObject<IndexNode>,
        store: ShaderNodeObject<StorageBufferNode>,
      ) => {
      ...
      const computeHeight = Fn(() => {
        ...
        const x = float(instanceIndex.mod(waterTextureSize)).mul(1 / waterTextureSize)
        const y = float(instanceIndex.div(waterTextureSize)).mul(1 / waterTextureSize)
        const center = vec2(0.5)

        const player = uPlayerPosition.toVar()
        const playerPos = vec2(player.x, player.z)
        const speedLen = uSpeedSmooth // 0..1
        const bounceDown = sin(player.w).negate().max(0.0)

        const posInPlane = vec2(x, y).sub(center).mul(PLANE_SIZE)
        const dist = length(posInPlane.sub(playerPos))
        ...
        If(dist.greaterThan(waterRippleRadius), () => {
          ...
          const newHeight = neighborHeight.sub(prevHeight).mul(WATER_VISCOSITY).toVar()
          const phase = clamp(dist.mul(Math.PI).div(WATER_IMPACT_SIZE), 0.0, Math.PI)
      ...
      const getFractalColor = Fn(([uv]: [uv: ShaderNodeObject<VarNode>]) => {
        ...
        Loop(octaves, () => {
          result.addAssign(amplitude.mul(mx_fractal_noise_float(p).mul(0.5).add(0.5)))
          amplitude.mulAssign(diminish)
          p.mulAssign(lacunarity)
        })

        return result.mul(0.28)
      }).setLayout({
      ...
      const getGodRays = Fn(
        ([position, distanceFromCenter]: [
          ...
          Loop(godRaySamples, ({ i }) => {
            ...
            illuminationDecay.mulAssign(0.6)
          })
      ...
      const getPulsesColour = Fn(
        ([distanceFromCenter]: [distanceFromCenter: ShaderNodeObject<VarNode>]) => {
          const ringSpeed = uPulsesSpeed.mul(time)

          const rings = sin(distanceFromCenter.mul(uPulsesFrequency).sub(ringSpeed))
      ...
      const getHeightColour = Fn(([vertexIndex]: [number]) => {
      ...
      const colorNode = Fn(() => {
        ...
        const backgroundNoise = mx_noise_float(screenUV.mul(8.0).add(time.mul(0.14)))
          .mul(0.5)
          .add(0.5)
          .toVar()
        ...
        const noiseIntensity = smoothstep(radius, radius.sub(softness), distanceFromCenter)
        ...
        finalColour.addAssign(godRays.add(uLightAmount.mul(0.16)))
        ...
        // Ripple height-based colouring
        const heightColour = getHeightColour(vertexIndex)
        finalColour.xyz.addAssign(heightColour)
  ...
  return (
    <>
      <mesh position={[0, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry
          args={[PLANE_SIZE, PLANE_SIZE, waterTextureSize - 1, waterTextureSize - 1]}
        />
