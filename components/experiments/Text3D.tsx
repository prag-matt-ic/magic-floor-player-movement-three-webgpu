...
import { TextGeometry, TextGeometryParameters } from 'three/examples/jsm/Addons.js'
import {
  color,
  cos,
  float,
...
const Text3D: FC = () => {
  ...
  const { textGeometry, positionNode, colorNode } = useMemo(() => {
    ...
    const positions = instancedArray(textGeometry.attributes.position.array, 'vec3')

    const positionStorage = storage(new StorageBufferAttribute(count, 3), 'vec3')
