...
import { rotate2D } from "./tslHelpers";
...
export const Particles: FC = () => {
  ...
  const { key, positionNode, colorNode, opacityNode, scaleNode } =
    useMemo(() => {
      ...
      const initPositions = Fn(() => {
        ...
        const noise2 = mx_noise_float(
          vec3(seed.add(0.5), idxFloat.mul(0.7), seed.mul(2.34))
        );
        const noise3 = mx_noise_float(
          vec3(seed.mul(0.8), idxFloat.add(1.0), seed.add(3.45))
        );
      ...
      const positionNode = Fn(() => {
        ...
        position.z.subAssign(
          sin(driftPhase.add(quarterTurn)).mul(lateralStrength)
        );
      ...
      const colorNode = Fn(() => {
        const particleDistanceFromCenter = distance(
          positionWorld.xz,
          vec2(0.0)
        );
      ...
      const opacityNode = Fn(() => {
        ...
        const fadeOut = float(1.0).sub(
          smoothstep(period.sub(fadeDuration), period, tCycle)
        );
