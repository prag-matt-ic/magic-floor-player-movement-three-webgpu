...
export const PLANE_RADIUS = 8
export const PLANE_SIZE = PLANE_RADIUS * 2

export const PLAYER_RADIUS = 0.14
export const PLAYER_HEIGHT = PLAYER_RADIUS + 0.01 // sits just above the plane
...
export const CENTER_RADIUS = PLAYER_RADIUS * 5.0
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  userHasSetQuality: false,
  setQualityMode: (qualityMode) => {
    set({
      qualityMode,
      config: PERFORMANCE_PRESETS[qualityMode],
      userHasSetQuality: true,
    })
  },
  onPerformanceChange: (up: boolean) => {
    if (get().userHasSetQuality) return // don't auto-change if user has set manually
    if (get().stage !== Stage.LANDING) return // only auto-change during landing stage

    const order = Object.values(QualityMode).reverse() // [Low, Medium, High, Max]

    set((state) => {
      const idx = order.indexOf(state.qualityMode)
      const nextIdx = Math.min(order.length - 1, Math.max(0, idx + (up ? 1 : -1)))
